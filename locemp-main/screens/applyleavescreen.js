...
export default function ApplyLeave() {
  ...
  // Ensure eid is fetched from the context
  const eid = employeeData?.eid;
  ...
  const submitLeaveRequest = async () => {
    ...
    try {
      // Use setDoc to specify the document ID as the employee's ID
      await setDoc(doc(db, 'leaveRequests', eid), leaveData);
      Alert.alert(
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title={startDate.toLocaleDateString()} onPress={() => setShowStartDatePicker(true)} />
      {showStartDatePicker && (
        <DateTimePicker
          value={startDate}
          mode="date"
      ...
      <Button title={endDate.toLocaleDateString()} onPress={() => setShowEndDatePicker(true)} />
      {showEndDatePicker && (
        <DateTimePicker
          value={endDate}
          mode="date"
