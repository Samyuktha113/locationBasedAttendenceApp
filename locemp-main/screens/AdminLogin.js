...
export default function LoginScreen({ navigation }) {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const employeeDoc = await getDoc(doc(db, "admins", employeeId));
...
//                 <Text style={{ color: '#1D4ED8', marginTop: 10, textDecorationLine: 'underline' }}>
//                     Forgot Password?
//                 </Text>
//             </TouchableOpacity>
...
export default function LoginScreen({ navigation }) {
  ...
  //     headerLeft: () => null,
  //     headerShown: true, // Removes the back arrow
  //     gestureEnabled: false, 
  //     title: "", // Disables the swipe-back gesture on iOS
  //   });
  // }, [navigation]);
  ...
  const handleLogin = async () => {
    ...
    try {
      const employeeDoc = await getDoc(doc(db, "admins", employeeId.trim()));
      if (!employeeDoc.exists()) {
        setError("Invalid Employee ID");
        setIsLoading(false);
        return;
